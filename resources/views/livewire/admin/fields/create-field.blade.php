<div class="max-w-2xl">
    <div class="mb-6">
        <h2 class="page-title">Create New Field</h2>
        <p class="page-subtitle">Add a new field to the monitoring system</p>
    </div>
    ...
    <form wire:submit="save" class="card">
        <div class="card-body space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input wire:model="name" type="text" class="form-input" placeholder="e.g. North Block A">
                    @error('name') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="form-label">Crop Type *</label>
                    <input wire:model="crop_type" type="text" class="form-input" placeholder="e.g. Maize">
                ...
                <div>
                    ...
                    <input wire:model="area_hectares" type="number" step="0.1" class="form-input" placeholder="e.g. 5.5">
                    @error('area_hectares') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="sm:col-span-2">
                    <label class="form-label">Location</label>
                    <input wire:model="location" type="text" class="form-input" placeholder="e.g. Nakuru, Rift Valley">
                    @error('location') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div class="sm:col-span-2">
                    <label class="form-label">Description</label>
                    <textarea wire:model="description" rows="3" class="form-textarea" placeholder="Notes about this field..."></textarea>
        ...
        <div class="px-6 py-4 border-t border-gray-800 flex items-center gap-3">
            ...
            <a href="{{ route('admin.fields.index') }}" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
